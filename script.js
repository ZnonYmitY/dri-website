...
const conceptDetails = {
  ...
  rag: {
    title: "RAG",
    image: { src: "assets/concept-rag-reference.png", alt: "RAG 检索增强生成概念图" },
  ...
  agent: {
    title: "Agent",
    image: { src: "assets/concept-agent-reference.png", alt: "Agent 模块概念图" },
...
const dimensionDetails = [
  {
    title: "机器学习与深度学习",
    intro: "能否为业务场景选择合适的模型架构并评估训练效果。",
    questions:
      "入门：了解 CNN/Transformer/SFT 等基本概念，能读懂评估报告、参与选型讨论并配合提供标注数据，但不能主导。\n不错：理解各架构优劣与训练质量标准，能独立制定模型选型策略、设计 A/B 评测体系，并与算法对齐迭代节奏。\n精通：吃透训练全流程与前沿可落地性，能从产品目标反推模型能力、设计端到端迭代飞轮，输出选型方法论。",
  },
  {
    title: "强化学习",
    intro: "能否通过奖励机制和策略优化让模型输出更符合人类偏好。",
    questions:
      "入门：了解 RLHF 原理及 DPO/GRPO 定位差异，能理解对齐的产品价值并配合定义好/坏回复标准。\n不错：掌握 RLHF/DPO/GRPO 的优劣与适用条件，能主导对齐策略、把产品调性转化为奖励信号并做 SFT vs RL 的 ROI 判断。\n精通：深度理解奖励信号的多目标权衡与前沿对齐研究，能设计「目标→奖励→行为」端到端闭环并定义团队对齐方法论。",
  },
  {
    title: "多模态",
    intro: "能否在成本可控的前提下设计流畅的视觉与语音交互体验。",
    questions:
      "入门：能分清识图/生图，了解 ASR/TTS 概念及延迟成本约束，能参与评审并对比竞品能力。\n不错：理解 ViT/CLIP/Diffusion 原理与 TTFT 等关键指标，能做精度、延迟、成本三角的选型，制定延迟 SLA 并主导视觉/语音 PRD。\n精通：把握多模态融合前沿与新模态落地节点，能设计跨模态创新交互、做全局架构并输出多模态产品方法论。",
  },
  {
    title: "Agent 与工作流编排",
    intro: "能否合理拆分工作流节点并规划智能体的行为边界。",
    questions:
      "入门：了解 Agent 感知、规划、执行流程及串/并行工作流，能画简单流程图、描述各环节输入输出。\n不错：掌握工作流引擎编排逻辑与工具调用模式，能设计 Agent 行为边界与兜底策略、主导系统设计并定义能力分层路由。\n精通：精通 ReAct、Plan-and-Execute、Multi-Agent 等复杂架构，能设计可扩展架构、定义质量度量体系并输出 Agent 设计范式。",
  },
  {
    title: "上下文记忆管理",
    intro: "能否构建合理的记忆分层策略并确保召回的准确率与低延迟。",
    questions:
      "入门：了解上下文窗口限制、RAG 含义及短/长期记忆概念，能从用户视角提出「记住什么、忘记什么」的诉求。\n不错：理解 KV Cache 与 RAG 召回、排序、混合检索，能设计记忆分层架构与读写遗忘规则并主导记忆系统设计。\n精通：精通多种 Memory Provider 与索引策略，能设计跨会话统一记忆架构、做长期演进规划并将记忆产品化为壁垒。",
  },
  {
    title: "AI 设计能力",
    intro: "能否建立高质量的提示词体系并形成产品评测迭代闭环。",
    questions:
      "入门：了解 Prompt 基本写法、System Prompt 作用与安全对齐必要性，能按模板写简单 prompt 并识别明显 bad case。\n不错：掌握 Few-shot、CoT、ReAct 等系统方法与 System Prompt 分层，能设计完整提示词体系、A/B 框架与质量评分标准。\n精通：吃透 prompt 与模型能力的交互及安全攻防，能设计整体体验架构、制定跨产品线设计规范并输出 AI 设计方法论。",
  },
  {
    title: "AI 工程化与工具链",
    intro: "能否选用合适的基础设施推动工作流的工具化与自动化。",
    questions:
      "入门：了解 Skills/MCP/CLI 等基本概念，能用现有 AI 工具完成日常任务并向团队推荐好用工具。\n不错：理解 MCP 协议与自动化设计模式，能评估选用 AI 基础设施、设计团队工作流并做工具 ROI 量化评估。\n精通：精通 AI 基础设施架构与演进方向，能规划组织级工具化战略、推动工具与业务深度融合并输出工效提升方法论。",
  },
];
